...
## D-038 — Knowledge layer uses internal entity IDs, QIDs as external links

**Decision:** Every polity gets a History Atlas entity ID
(`ha:polity:<ascii-slug>`), generated by the isolated `slug.ts`. The
Wikidata QID is stored only as `wikidata_id` (an external knowledge-source
link), so the QID is never a primary key (work order §2.1). Polygon features
are linked to entities by normalized NAME at runtime; geometry files stay
untouched (§2.5).

## D-039 — Entity resolution and knowledge fetch are build-time only

**Decision:** `scripts/match-wikidata.ts` resolves entities against Wikidata
at build time; `scripts/build-knowledge.ts` pre-fetches structured facts +
Wikipedia summaries into static JSON. Runtime (`knowledgeService.ts`) only
reads `/data/entities.json` and `/data/knowledge/entities-knowledge.json`
with an in-memory cache — no per-click API calls (§B-5, §C-6), no backend.
All raw API responses are cached so re-runs are deterministic and
rate-limit-friendly; a failed request preserves the previous registry value.

## D-040 — Auto-matching never hard-confirms weak signals

**Decision:** Matches score name/alias (0.5), polity-type (0.25, P31 inside
the P279* closure of historical-country/country/state/empire), and temporal
plausibility (0.25, P571/P576 vs. the dataset span ±150 y). Auto matches cap
at `probable` unless the score is >= 0.9 with an exact label, polity-type >=
0.6, and temporal agreement; near-tie candidates become `ambiguous` with all
candidates stored; humans (Q5) can never match. `confirmed` without human
review only comes from `data/entity-overrides.json` (§B-3, §B-4).
